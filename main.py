...
rahasia_list = ['🪙 Lempar koin –  tampilkan "Kepala" atau "Ekor"',
                "🔐 Generator kata sandi - hasilkan kata sandi acak!",
                "🎨 Gambar acak - tunjukkan meme, hewan, atau gambar luar angkasa kepada pengguna!",
                "🧙 Teka-teki - buat teka-teki yang menarik dan desain yang keren!"]

@app.route("/")
def home():
    return f'<h1>Selamat datang di website saya, silahkan klik link dibawah untuk melihat fakta menarik</h1> <a href="/facts">Fakta menarik!</a>'
...
@app.route('/about')
def about():
    return '<h1>Website ini dibuat untuk memenuhi tugas praktikum pemrograman web</h1>'

@app.route("/rahasia")
def rahasia():
    return f'<h1>{random.choice(rahasia_list)}</h1>'
